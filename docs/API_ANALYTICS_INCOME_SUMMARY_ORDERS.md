...
- `totalCollateralPlanExpectedToRefund` / `totalCollateralPlan`: **Tiền thế chân dự kiến** — số tiền thế chân dự kiến trả trong tương lai đối với đơn hàng dự kiến trả trong kỳ.
...
| **totalCollateralPlanExpectedToRefund** | **Tiền thế chân dự kiến** — số tiền thế chân dự kiến trả trong tương lai cho đơn hàng dự kiến trả trong kỳ. | Tổng tiền cọc (thế chân) sẽ trả lại khách khi các đơn thuê có **lịch trả hàng** rơi vào kỳ được khách trả hàng. Dùng để **chuẩn bị thanh khoản** (biết sắp phải trả bao nhiêu). |
...
| **totalCollateralPlan** | Tiền thế chân dự kiến trả trong tương lai — đơn có lịch trả hàng rơi vào ngày đó. | |
...
| **RENT_PICKUP** | Thu tiền khi khách lấy hàng. | Khách lấy hàng (PICKUPED): thu tiền thuê (totalAmount) + cọc thế chân (securityDeposit); nếu đã thu cọc trước thì trừ depositAmount. | Dương |
| **RENT_RETURN** | Khách trả hàng — xử lý cọc và phí. | Khách trả hàng (RETURNED): hoàn cọc (securityDeposit) trừ phí hư hỏng (damageFee). Nếu damageFee > securityDeposit thì thu thêm → revenue âm (thu tiền). | Thường dương (hoàn cọc); có thể âm nếu thu phí hư hỏng > cọc |

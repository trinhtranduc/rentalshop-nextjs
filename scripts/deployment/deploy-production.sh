...
if ! railway add --database postgres --service database 2>/dev/null; then
...
sleep 1
railway variables --set DATABASE_URL='${{database.DATABASE_URL}}' --service apis
sleep 1
...
sleep 1
railway variables --set CORS_ORIGINS='https://client-production-d10a.up.railway.app,https://admin-production-89d0.up.railway.app' --service apis
...
sleep 1
railway variables --set NEXT_PUBLIC_API_URL='https://apis-production-b698.up.railway.app' --service client
sleep 1
...
sleep 1
railway variables --set NEXT_PUBLIC_API_URL='https://apis-production-b698.up.railway.app' --service admin
sleep 1
...
railway run --service apis bash -c "cd ../../ && npx prisma db push --accept-data-loss"

# Generate Prisma client
log_info "Generating Prisma client..."
railway run --service apis bash -c "cd ../../ && npx prisma generate"

# Seed database
log_info "Seeding production database..."
railway run --service apis bash -c "cd ../../ && yarn db:regenerate-system"
...
echo "  API:    https://apis-production-b698.up.railway.app"
echo "  Client: https://client-production-d10a.up.railway.app"
echo "  Admin:  https://admin-production-89d0.up.railway.app"
echo ""

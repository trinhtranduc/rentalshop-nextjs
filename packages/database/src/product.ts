...
export async function searchProducts(filters: ProductSearchFilter) {
  ...
  const transformedProducts = products.map((product: any) => {
    ...
    return {
      ...
      embeddingGeneratedAt: product.embeddingGeneratedAt,
...
async function getOrCreateDefaultCategory(merchantId: number): Promise<any> {
  const { DEFAULT_CATEGORY_NAMES, getOnboardingCopy } = await import('./onboarding-seed-i18n');

  // Prefer isDefault, then known localized default names (General / Chung)
  const existingDefault =
    ...
    (await prisma.category.findFirst({
      where: {
        merchantId,
        name: { in: [...DEFAULT_CATEGORY_NAMES] },
        isActive: true,
      },
    }));
  ...
  // Create default category if not exists (Vietnamese default — primary market)
  console.log('🔧 Creating default category for merchant:', merchantId);
  const { category } = getOnboardingCopy('vi');
...
export async function createProduct(input: any): Promise<any> {
  ...
  // Create product - let database auto-generate ID to avoid race conditions

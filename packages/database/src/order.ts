  ...
  if (newStatus && newStatus !== oldStatus && orderType && oldOutletId && oldOrderItems.length > 0) {
    try {
      ...
      await updateOutletStockForOrder(
        ...
        oldOutletId,
        oldOrderItems.map((item: { productId: string | null; quantity: number }) => ({
...
export const simplifiedOrders = {
  ...
  findManyMinimal: async (filters: {
    ...
    const itemCountMap = new Map(itemCounts.map((item: { orderId: string; _count: { id: number } }) => [item.orderId, item._count.id]));
    ...
    const paymentCountMap = new Map(paymentCounts.map((payment: { orderId: string; _count: { id: number } }) => [payment.orderId, payment._count.id]));
    const totalPaidMap = new Map(paymentCounts.map((payment: { orderId: string; _sum: { amount: number | null } }) => [payment.orderId, payment._sum.amount || 0]));
  ...
  getStatistics: async (filters: {
    ...
    return {
      ...
      statusBreakdown: statusBreakdown.reduce((acc: Record<string, number>, item: { status: string; _count: { id: number } }) => {
        acc[item.status] = item._count.id;
        return acc;
      }, {} as Record<string, number>),
      typeBreakdown: typeBreakdown.reduce((acc: Record<string, number>, item: { orderType: string; _count: { id: number } }) => {

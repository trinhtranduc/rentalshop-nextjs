//
//  SaleDetailCell_Option4_AccentBorder.swift
//  POS ADBD
//
//  OPTION 4: Modern với Left Accent Border
//  Design: Colored accent line bên trái theo status, clean và modern
//

import UIKit
import SnapKit

class SaleDetailCell_Option4: UITableViewCell {
    // MARK: - UI Components
    ...
    // Left accent border - màu theo status
    private lazy var accentBorderView: UIView = {
        let view = UIView()
        view.backgroundColor = APP_TONE_COLOR // Default color
        return view
    }()
    ...
    private func setupUI() {
        ...
        containerView.addSubview(accentBorderView)
        ...
        // Accent border - 4px width, full height
        accentBorderView.snp.makeConstraints { make in
    ...
    func bind(order: Order, sortType: OrderSortType = .book_date) {
        ...
        updateAccentBorderColor(for: order)
        applyDateStyling(sortType: sortType, orderType: order.orderType)
    }
    
    private func updateAccentBorderColor(for order: Order) {
        if order.orderType == .rent {
            switch order.status {
            case .reserved:
                accentBorderView.backgroundColor = .red
            case .pickuped:
                accentBorderView.backgroundColor = APP_ORANGE_COLOR
            case .returned:
                accentBorderView.backgroundColor = APP_TONE_COLOR
            case .cancelled:
                accentBorderView.backgroundColor = UIColor(hexString: "b22222")
            default:
                accentBorderView.backgroundColor = .systemGray4
            }
        } else {
            switch order.status {
            case .completed:
                accentBorderView.backgroundColor = .actionSuccess  // Green color like print button
            case .reserved:
                accentBorderView.backgroundColor = APP_ORANGE_COLOR
            case .cancelled:
                accentBorderView.backgroundColor = UIColor(hexString: "b22222")

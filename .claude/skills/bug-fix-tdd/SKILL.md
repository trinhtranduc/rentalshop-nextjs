...
0. **Issue first.** Open or reuse a GitHub issue (`issue-first`) and write
   `.agent/changes/<issue>-<slug>/` before the fix branch. The PR body must contain `Fixes #<issue>`.
...
5. **Fix the implementation** at the root cause. Check "Things agents get wrong" in `AGENTS.md` first. Most
...
9. **Commit** `fix(<scope>): <what changed>`. If this is the second time the mistake occurred, add a line under
   "Things agents get wrong" in `AGENTS.md` in the same PR, plus an eval case (`incident-to-eval`).
10. **Hotfix branches** are `hotfix/<issue>-<slug>` off `main`; PR to `main`, then back-merge into `dev`.
